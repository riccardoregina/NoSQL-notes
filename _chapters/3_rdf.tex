...
\begin{center}
    \code{"lexical form"\textasciicircum\textasciicircum language-tag}
\end{center}
...
\begin{definition}{Basic Graph Pattern}
    A \keyword{basic graph pattern} (BGP) is a set of triple patterns, interpreted as in conjunction (i.e., all triple patterns must match simultaneously).
\end{definition}
...
\begin{definition}{Group Graph Pattern}
    A group graph pattern (GGP) is a pattern expressed inside a pair of curly braces \code{\{...\}}.
\end{definition}

GGPs are combined using the \code{Join} operation: %explain
...
\begin{definition}{Solution Multiset}
    A \keyword{solution multiset} is a multiset\footnote{Compared to a set, a multiset allows multiple occurrences of identical elements. This means that, in a multiset, each item is associated with a cardinality.} of solution mappings.
\end{definition}

\begin{definition}{Solution}
    Given an RDF graph \(G\) and a BGP \(P\), a solution mapping \(\mu\) is a \keyword{solution} if it is defined exactly on the variables in \(P\) and there exists a mapping \(\sigma\) from blank nodes to RDF terms such that \(\mu(\sigma(P))\subseteq G\), i.e. \(\mu(\sigma(P))\) is a subgraph of \(G\).
    
    The \textbf{cardinality} of \(\mu\) in the solution multiset is the number of distinct \(\sigma\) mappings that satisfy the condition.
\end{definition}

We denote the multiset as \(\text{eval}_G(P)\). Now, let's examine an example to better understand the notion of cardinality.
...
# output: true
\end{lstlisting}
Such a query can admit only two solutions: \(\set{}\) (false: the pattern did not match) and \(\set{\emptyset}\) (true: the pattern did match, where \(\emptyset\) denotes the empty mapping).
...
\begin{definition}{\(\Path{}\)}
    ...
    \Path{PP} = \begin{cases}
        ...
        \Path{PP} \setminus \bigcup_{i=1}^{n}\Reversed{\Path{\text{iri}_i}} & PP=!(\text{\textasciicircum iri}_1|\ldots | \text{\textasciicircum iri}_n) \\[4pt]
        ...
        \Path{PP_1} \cup \Path{PP_2} & PP=PP_1|PP_2
    \end{cases}
    \]
...
The official SPARQL 1.1 algebra says that property paths built with \code{*}, \code{+}, \code{?} are evaluated via an ALP (Arbitrary Length Path) algorithm, which is essentially a graph-reachability search. So, ALP searches if a path from \(s\) to \(o\) exists and does not search for all possible paths between these two nodes. 
...
\begin{definition}{Wikidata Entity Document}
    ...
    \begin{itemize}
        ...
        \item \emph{Statements}: sourced claims\footnote{A Wikidata claim is a subject-property-value triple.} for several properties that the entity might have, with a rank (normal, preferred, deprecated) to encode their current significance (preferred statements are marked as the most important; deprecated ones are known to be wrong but kept for reference), qualifiers (context information like 
        \q{excluding non-official matches}), references (one or more sources backing up the claim). For example, \code{Q17515} \(\to\) \q{member of sports team} \(\to\) \q{SSC Napoli};
...
\begin{enumerate}
    \item Wikidata statements have \keyword{annotations}, property-value pairs that add information to the statement;
    \item Wikidata property types do not correspond to XML Schema types; 
    \item Wikidata IDs are not immediately IRIs.
\end{enumerate}
However, Wikidata and RDF have some similarities: 
\begin{enumerate}
    \item Both are directed labeled graphs;
    \item All data that appear inside a statement can be subjects/objects in other statements: Wikidata is a hypergraph;
    \item Statements order does not matter.
\end{enumerate}

To represent annotations, we reify the overall statement into a standalone entity and then connect it to the annotations by creating a triple for each annotation as object and the overall statement as subject.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}llp{5cm}@{}}
        ...
        \texttt{wikibase:} & \texttt{http://wikiba.se/ontology\#} & \texttt{wikibase:rank} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{example}{}
    Navigate to \url{https://query.wikidata.org/} and type the following SPARQL query to find out all the players who have played with D.~A.~Maradona.
    \begin{lstlisting}[language=sql]
...
SELECT DISTINCT ?playerLabel ?clubLabel WHERE {
 wd:Q17515 wdt:P54 ?club . -- Q17515 is Maradona's id; P54 is the 'member of sports team' property id.
 ?player wdt:P54 ?club .
