...
\begin{note}{Revisiting CAP Theorem}
    ...
    A primary source of confusion lies in the misalignment between CAP properties and the homonymous ACID guarantees. 
    Within ACID, the 'C' denotes transactional invariants --- such as the preservation of unique keys --- whereas in CAP it refers exclusively to single-copy consistency, a far narrower constraint. 

    Beyond this terminological discord, the classic binary interpretation of CAP fundamentally misrepresents its properties. 
    Consistency, availability, and even partitions themselves are continuous spectra, not boolean toggles. Availability ranges from 0 to 100\%, consistency admits numerous gradations, and the perception of a partition can vary asymmetrically across nodes. 
    ...
    This nuanced view realigns the relationship between CAP and the BASE design philosophy. 
    BASE was explicitly crafted to embody the high-availability end of the spectrum, in direct contrast to ACID's consistency-centric approach. 
    Soft state and eventual consistency are particularly effective techniques for maintaining availability during partitions, and contemporary large-scale systems pragmatically blend elements from both ACID and BASE rather than committing to a single pole.
